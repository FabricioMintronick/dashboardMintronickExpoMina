...
for(const file of ['encoder.png','sensor-lineal.png'])if(!fs.existsSync(path.join('public','assets','sensors',file)))throw new Error(`Missing sensor image: ${file}`);
for(const name of ['mintronick-operaciones.apk','mintronick-collector.apk']){const apk=path.join('public','downloads',name);if(!fs.existsSync(apk)||fs.statSync(apk).size<10000)throw new Error(`Missing or invalid Android APK: ${name}`);}
