...
function registerTrackRoutes(app,{getDb,collection}){
  app.get('/api/telemetry/track',async(req,res)=>{
    ...
    try{
      ...
      const points=rows.map(r=>({...r.point,lat:numeric(r.point.lat),lon:numeric(r.point.lon),speed:numeric(r.point.speed)})).filter(p=>p.lat!==null&&p.lon!==null&&Math.abs(p.lat)<=90&&Math.abs(p.lon)<=180&&!(p.lat===0&&p.lon===0));
