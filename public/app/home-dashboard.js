...
import {attentionReason,operationalStateIndex,stateDefinitions} from './home-insights.js';
...
export function commandCenter(page,{items}){
  ...
  function popup(item){const speed=item.metrics?.speed,hasSpeed=speed?.quality==='fresh'&&Number.isFinite(speed.value),moving=hasSpeed&&speed.value>1;return `<div class="home-equipment-popup"><strong>${e(item.name)}</strong>${state(item)}<span>${hasSpeed?`${number(speed.value)} ${e(speed.unit)} · ${moving?'En movimiento':'Sin desplazamiento GPS'}`:'Velocidad sin lectura'}</span><small>${e(date(item.location?.at||item.lastAt))}</small><button type="button" class="button" data-popup-quick="${e(item.id)}">Ficha rápida</button><a class="button secondary" href="#map/${encodeURIComponent(item.id)}">Abrir mapa</a></div>`;}
  function highlight(){page.querySelectorAll('#home-map-list [data-home-focus]').forEach(button=>button.classList.toggle('selected',button.dataset.homeFocus===selectedId));}
  function focus(item){if(!item?.location)return;selectedId=item.id;openPopupId=item.id;highlight();map.setView([item.location.lat,item.location.lon],17);drawMap();markerById.get(item.id)?.openPopup();}
  function drawList(){const signature=current.map(item=>`${item.id}:${stateInfo(item).key}:${Boolean(item.location)}`).join('|');if(signature===lastListSignature){highlight();return;}lastListSignature=signature;const list=page.querySelector('#home-map-list'),scroll=list.scrollTop;list.innerHTML=current.map(item=>{const info=stateInfo(item);return `<button class="map-item state-${info.key}" data-home-focus="${e(item.id)}" ${item.location?'':'disabled'}><span><i class="map-state-dot state-${info.key}"></i><strong>${e(item.name)}</strong></span><span class="map-state-chip state-${info.key}">${e(info.label)}</span></button>`;}).join('');list.scrollTop=scroll;list.querySelectorAll('[data-home-focus]').forEach(button=>button.onclick=()=>focus(current.find(item=>item.id===button.dataset.homeFocus)));highlight();}
  function drawMap(){
    ...
    for(const group of groups){if(group.items.length>1&&map.getZoom()<15){const marker=L.marker([group.items[0].location.lat,group.items[0].location.lon],{icon:L.divIcon({className:'geo-cluster',html:`<b>${group.items.length}</b>`,iconSize:[38,38],iconAnchor:[19,19]})}).addTo(markers);marker.bindTooltip(`${group.items.length} equipos`);marker.on('click',()=>map.fitBounds(L.latLngBounds(group.items.map(item=>[item.location.lat,item.location.lon])),{maxZoom:17,padding:[45,45]}));continue;}
      group.items.forEach(item=>{const info=stateInfo(item),marker=L.marker([item.location.lat,item.location.lon],{icon:L.divIcon({className:`geo-machine home-map-machine state-${info.key}`,html:'<img src="/tractor-d8-transparent.png" alt="">',iconSize:[40,30],iconAnchor:[20,15]})}).addTo(markers);markerById.set(item.id,marker);marker.bindTooltip(e(item.name),{permanent:true,direction:'top',className:`equipment-map-label state-${info.key}`});marker.bindPopup(popup(item),{className:'equipment-map-popup',maxWidth:260});marker.on('click',()=>{selectedId=item.id;openPopupId=item.id;highlight();});marker.on('popupopen',event=>{selectedId=item.id;openPopupId=item.id;highlight();event.popup.getElement()?.querySelector('[data-popup-quick]')?.addEventListener('click',()=>openEquipment(item),{once:true});});marker.on('popupclose',()=>{if(openPopupId===item.id)openPopupId=null;});});
    }
