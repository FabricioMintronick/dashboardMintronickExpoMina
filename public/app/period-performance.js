...
export function counterDelta(data){
  const points=(data?.points||[]).filter(point=>finite(point.v));
  if(points.length<2)return {value:null,reason:'Faltan lecturas inicial y final'};
