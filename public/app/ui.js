...
let dialog, returnFocus, quickId;
...
export function openEquipment(item){
  ...
  returnFocus=document.activeElement;quickId=item.id;
  dialog.innerHTML=`<div class="quick-head"><div><small>FICHA RÁPIDA</small><h2>${e(item.name)}</h2></div><button class="button secondary" data-close aria-label="Cerrar ficha">✕</button></div><div class="quick-body"><img src="/tractor-d8-transparent.png" alt="Tractor de orugas"><div data-quick-status>${state(item)} ${quality(item.communication)}<p>${condition(item)}</p><small>${e(item.gateway)} · Últimas lecturas</small></div></div><div class="quick-metrics">${['hours','rpm','fuel','coolant','speed','voltage'].map(key=>{const m=item.metrics[key];return `<div><small>${e(m.label)}</small><strong>${number(m.value)} <em>${e(m.unit)}</em></strong>${metricQuality(m)}</div>`;}).join('')}</div>${item.alarms.length?`<button class="button secondary quick-alert-action" data-notifications data-notification-equipment="${e(item.id)}">${icon('alerts')} Ver ${item.alarms.length} ${item.alarms.length===1?'aviso':'avisos'}</button>`:''}<div class="quick-actions"><a class="button" href="#equipment/${encodeURIComponent(item.id)}">Ficha completa →</a><a class="button secondary" href="#map/${encodeURIComponent(item.id)}">${icon('gps')} GPS</a><a class="button secondary" href="#history/${encodeURIComponent(item.id)}">${icon('history')} Historial</a></div>`;
...
export function observeAlarms(items){
  ...
  if(dialog?.open){const item=items.find(i=>i.id===quickId);if(item){
    dialog.querySelector('[data-quick-status]').innerHTML=state(item)+' '+quality(item.communication)+'<p>'+condition(item)+'</p><small>'+e(item.gateway)+' · Últimas lecturas</small>';
    ...
    let alarmButton=dialog.querySelector('[data-notification-equipment]');if(!alarmButton){alarmButton=document.createElement('button');alarmButton.className='button secondary quick-alert-action';alarmButton.dataset.notifications='';alarmButton.dataset.notificationEquipment=item.id;dialog.querySelector('.quick-actions').before(alarmButton);}alarmButton.hidden=!item.alarms.length;alarmButton.innerHTML=icon('alerts')+' Ver '+item.alarms.length+' '+(item.alarms.length===1?'aviso':'avisos');
  }}
