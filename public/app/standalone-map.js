...
import {mount} from './map-explorer.js?v=20260904-4';
...
load();const timer=setInterval(async()=>{if(document.hidden||!cleanup.update)return;try{const fleet=await getJSON('/api/fleet');cleanup.update(fleet.equipment.filter(i=>!i.test));}catch{cleanup.setGpsAvailable?.(false);}},2000);window.addEventListener('beforeunload',()=>{clearInterval(timer);cleanup();});
