...
export function detailView(page,{item,panel}) {
  ...
  <section class="panel" id="detail-performance"></section>
  ${trendHTML([item],item)}
  ...
  const trend=mountTrend(page,[item]),accumulated=mountAccumulatedCharts(page.querySelector('.accumulated-panel'),item),performanceEnd=new Date(item.lastAt||Date.now()),performance=mountPeriodPerformance(page.querySelector('#detail-performance'),{gateway:item.gateway,from:new Date(performanceEnd-86400000).toISOString(),to:performanceEnd.toISOString(),getJSON,controls:true});
